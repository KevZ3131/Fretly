...
export default function FretlyGuitar() {
  ...
  const [fretboard, setFretboard] = useState<Map<number, FretState>[]>(() => Array.from({ length: 6 }, () => new Map()))
  ...
  return (
    <div className="w-full">
      <div className="text-center mb-6">
        <h2 className="text-2xl font-bold text-white mb-2">Guitar Fretboard</h2>
                    ...
                    {Array.from({ length: FRETS + 1 }).map((_, fi) => {
                      ...
                      return (
                        <button
                          ...
                          onMouseEnter={() => {
                            updateFret(si, fi, { highlighted: true })
                          }}
                          onMouseLeave={() => {
                            updateFret(si, fi, { highlighted: false })
                          }}
                        >
                          <div
                              ...
                              backgroundColor: isSelected(si, fi)
                                ...
                                : isHighlighted(si, fi)
                                  ...
                                  : isVoicing(si, fi)
                                    ? "#f59e0b"
                                    : "#22c55e",
                              opacity: isSelected(si, fi) ? 1 : isHighlighted(si, fi) || isVoicing(si, fi) ? 0.85 : 0.3,
                              marginLeft: "auto",
                              marginRight: "auto",
            ...
            {DOT_FRETS.map((fret) => {
              ...
              return (
                <div
                  key={`dot-${fret}`}
                  style={{ left: `${left}%` }}
                  className="absolute transform -translate-x-1/2 w-0 h-full flex items-center justify-center"
                >
                  {dotElements.map((d, i) => (
                    <div
                      key={i}
                      style={{ top: d.top }}
                      className="absolute -translate-y-1/2 w-5 h-5 rounded-full bg-slate-200 shadow"
                    />
                  ))}
                </div>
              )
